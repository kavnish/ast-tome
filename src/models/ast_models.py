...
class ASTModel(nn.Module):
    ...
    def reduceToMe(self, x, reduce_fraction):
        print("tokens before reduction: ", x.shape)
        merge, _ = tome.merge.bipartite_soft_matching(x, 0)
        x = merge(x)
        # x = x[:, : int(x.shape[1] * 0.95), :] #random merge
        print("tokens after reduction: ", x.shape)
        return x
    ...
    def forward(self, x):
        ...
        x = self.v.pos_drop(x)
        for idx, blk in enumerate(self.v.blocks):
            x = blk(x)
            x = self.reduceToMe(x, 1)  # use a frac dictionary here
        x = self.v.norm(x)
        x = (x[:, 0] + x[:, 1]) / 2
